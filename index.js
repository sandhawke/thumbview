...
function setSize () {
  const w = document.getElementById('thumbs').clientWidth
  const cols = Math.floor(w/(vs.thumbsize + 4))
  if (cols != vs.cols) {
    vs.cols = cols
...
function thumbview2 (vs, slides, config) {
  ...
  // use config for onclick, img url, MAYBE actually for the element?
